...
export default function AuroraBackground({
    ...
    // Helper function to normalize color format (remove alpha channel if present)
    const normalizeColor = (hex: string): string => {
        // Remove any whitespace
        hex = hex.trim();

        // If it's 8 characters (with alpha), strip the last 2 characters
        if (hex.startsWith('#') && hex.length === 9) {
            return hex.substring(0, 7);
        }

        // If it's already 6 characters or 3 characters, return as is
        return hex;
    };
    ...
    useEffect(() => {
        ...
        const colorStopsArray = colorStops.map(hex => {
            ...
            const c = new Color(normalizedHex);
            return [c.r, c.g, c.b];
        });
        ...
        const update = (t: number) => {
            ...
            if (program) {
                ...
                // Color stops are already set in the program creation and don't need per-frame update unless they change dynamically
                // If dynamic updates are needed, they should be handled via a separate useEffect or ref, but for now removing from loop is safe as useEffect re-runs on prop change
